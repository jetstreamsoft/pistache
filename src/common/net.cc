...
namespace Pistache
{
    ...
    Address Address::fromUnix(struct sockaddr* addr)
    {
        ...
        if (family == AF_INET or family == AF_INET6 or family == AF_UNIX)
        {
            IP ip     = IP(addr);
    ...
    void Address::init(const std::string& addr)
    {
        ...
        if (isUnixDomain(addr))
        {
            ...
            auto size = std ::min(addr.size(), sizeof unAddr.sun_path);
            if (size == 0)
            {
            ...
            else if (addr[0] == '\0')
            {
                ...
                ::memcpy(unAddr.sun_path, addr.data(), size);
            }
            else
            {
                addrLen_ = static_cast<socklen_t>(
                    offsetof(struct sockaddr_un, sun_path) + size);
                ::strncpy(unAddr.sun_path, addr.c_str(), size);
